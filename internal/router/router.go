...
// RegisterProjectSchemaRoutes wires all schema endpoints.
//
// Schema routes split across two groups:
//
//   Flat schema operations (by schema ID):
//     POST   /api/v1/schemas              create a draft
//     GET    /api/v1/schemas?project_id=  list schemas for a project
//     GET    /api/v1/schemas/:id          get one schema
//     PUT    /api/v1/schemas/:id          update a draft's DDL
//     DELETE /api/v1/schemas/:id          delete a draft
//     PUT    /api/v1/schemas/:id/commit   draft → pending
//     GET    /api/v1/schemas/:id/status   get current state
//
//   Project-scoped schema shortcuts (by project ID):
//     GET    /api/v1/projects/:project_id/schemas/latest   highest version
//     GET    /api/v1/projects/:project_id/schemas/applied  currently applied
...
func (r *Router) RegisterProjectSchemaRoutes(h *handler.ProjectSchemaHandler) {
	...
	schemas := v1.Group("/schemas")
	{
		...
		schemas.GET("", h.GetSchemasByProject)       // ?project_id=xxx
	...
	// Project-scoped convenience routes — :project_id lives in the projects group
	projectSchemas := v1.Group("/projects/:project_id/schemas")
	{
