...
type ShardKeyInferenceResult struct {
	ProjectId string
	Decisions []ShardKeyDecision
}

func (s *InferenceService) RunForProject(ctx context.Context, projectId string) error {
	return s.runShardKeyInference(ctx, projectId)
}

func (s *InferenceService) runShardKeyInference(ctx context.Context, projectId string) error {
	cols, err := s.colRepo.GetColumnsByProjectId(ctx, projectId)
	if err != nil {
		return err
	}
	...
	logicalSchema, err := schema.BuildLogicalSchemaFromDb(projectId, cols, fkEdges)
	if err != nil {
		return err
	}
	shardKeyInferenceResult := BuildShardKeyPlan(logicalSchema)

	newShardKeyRecords := ConvertToShardKeyRecords(shardKeyInferenceResult.Decisions)

	if err := s.shardKeyRepo.ReplaceShardKeysForProject(ctx, projectId, newShardKeyRecords); err != nil {
		return err
	}
	return nil
}
